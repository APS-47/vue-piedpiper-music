...
<template>
  <div class="discover"
    ...
    <div class="songs-wrap">
      <h4>最新音乐</h4>
      <ul class="new-songs">
        <li v-for="(item,index) in newSongs"
            :key="index">
          <div class="music-img-wrap">
            <img v-lazy="item.picUrl"
                 alt="newSongs">
            <p class="iconfont icon-play"
               @click="playMusic(item)"></p>
          </div>
          ...
          <div class="music-info">
            ...
            <p class="music-singer"
               ...
               :key="i"
               style="display:inline;margin:0 5px">{{singer.name}}</p>
          </div>
        </li>
      </ul>
    </div>
